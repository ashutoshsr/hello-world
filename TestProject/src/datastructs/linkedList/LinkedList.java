...
public class LinkedList {
	...
	/*
	 * public ListNode remove(int position) { ListNode removedElement = null; if
	 * (position > length || position < 0) { return null; } else if (position ==
	 * length) { removeFromEnd(); return null; } else if (position == 0) {
	 * ListNode newHead = head.getNext(); head = null; head = newHead; length--;
	 * return head; } else { ListNode lastOne = head; ListNode previousOne =
	 * head; for (int i = 0; i < position; i++) { previousOne = lastOne; lastOne
	 * = lastOne.getNext(); } previousOne.setNext(lastOne.getNext());
	 * lastOne.setNext(null); length--; return lastOne; } }
	 */
	...
	public void swapNodes(int keyX, int keyY) {
